...
int FrameHandlerBase::finishFrameProcessingCommon(
    ...
    const UpdateResult dropout,//主函数中的res
...
#endif
  /* 如果返回值是RESULT_FAILURE，切当前工作状态为STAGE_DEFAULT_FRAME或STAGE_RELOCALIZING
   * 则使当前状态为STAGE_RELOCALIZING，并且tracking_quality_=TRACKING_INSUFFICIENT
   * 如果仅仅是RESULT_FAILURE，那么重置所有
   * 否则重置所有；
   * */
