...
//is_keyframe_标志位=true，然后调用setKeyPoints函数
  ...
  for(size_t i = 0; i < 5; ++i)
    if(key_pts_[i] != NULL)
      if(key_pts_[i]->point == NULL)
        key_pts_[i] = NULL;
  //找到图像中分布最明显的5个点,得到最靠近左上角，右上角，左下角，右下角，图像中心的5个点
  //BUG1:若点不均匀分布，则会出现没有如上5个点的问题，会有空指针bug
  std::for_each(fts_.begin(), fts_.end(), [&](Feature* ftr){ if(ftr->point != NULL) checkKeyPoints(ftr); });
}
//计算图像中的所有点，得到最靠近左上角，右上角，左下角，右下角，图像中心的5个点
